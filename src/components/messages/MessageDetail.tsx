  ...
  if (!activeCommunication) {
    return (
      <div className="hidden md:flex md:flex-1 md:flex-col md:items-center md:justify-center md:p-8 text-center">
        ...
        <h2 className="text-2xl font-semibold mb-2">No communication selected</h2>
        <p className="text-muted-foreground">Select a communication to view its details.</p>
      </div>
    );
  }
    ...
    <div className={`${activeCommunication ? 'flex' : 'hidden md:flex'} flex-1 flex-col overflow-hidden`}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        ...
        <div className="flex items-center gap-1">
          ...
          >
            <Star className={`h-5 w-5 ${activeCommunication.isStarred ? 'fill-yellow-400 text-yellow-400' : ''}`} />
          </Button>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={handleDeleteCommunication} className="text-red-500">
        ...
        <div className="flex items-start gap-3 mb-4">
          <Avatar className="h-10 w-10">
            {activeCommunication.sender.avatar ? (
              <AvatarImage src={activeCommunication.sender.avatar} alt={activeCommunication.sender.name} />
            ) : (
              <AvatarFallback>
                {activeCommunication.sender.name.split(" ").map(n => n[0]).join("")}
              </AvatarFallback>
            )}
          </Avatar>
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between">
              <div>
                <p className="font-medium">{activeCommunication.sender.name}</p>
                <p className="text-xs text-muted-foreground">
                  {activeCommunication.sender.role} • {formatTimestamp(activeCommunication.timestamp)}
                </p>
              </div>
              ...
              >
                {activeCommunication.type.charAt(0).toUpperCase() + activeCommunication.type.slice(1)}
              </Badge>
            </div>
            ...
            {activeCommunication.type === "assignment" && activeCommunication.dueDate && (
                ...
                <div key={comment.id} className="flex items-start gap-3">
                  <Avatar className="h-8 w-8">
                    {comment.sender.avatar ? (
                      ...
                      <AvatarFallback>
                        {comment.sender.name.split(" ").map(n => n[0]).join("")}
                      </AvatarFallback>
                    )}
                  </Avatar>
                  ...
                  <div className="flex-1">
                    <div className="flex items-baseline justify-between">
                      <p className="text-sm font-medium">{comment.sender.name}</p>
                      <p className="text-xs text-muted-foreground">
